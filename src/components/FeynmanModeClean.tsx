...
export function FeynmanMode({ onNavigate }: FeynmanModeProps) {
  ...
  const analyzeExplanation = async () => {
    ...
    try {
      ...
      console.log('Making K2 API call for analysis...');
      const prompt = studyPrompts.feynman({
        topic: `${selectedTopicData?.title || 'Unknown'} (${selectedTopicData?.subject || 'Unknown'})`,
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      <Button 
                        ...
                        disabled={isAnalyzing || explanation.length < 50}
                        variant="outline"
                        size="sm"
                      ...
                      >
                        ...
                        Analyze
                        {explanation.length >= 50 && <Sparkles className="w-3 h-3 ml-1 animate-pulse" />}
                      </Button>
                      <Button 
                        onClick={getSuggestions}
                        disabled={isGettingSuggestions || explanation.length < 50}
                        variant="outline"
                        size="sm"
                        className={`transition-all duration-300 hover:scale-105 hover:shadow-lg ${
                          explanation.length >= 50 
                      ...
                      >
                        ...
                        Suggestions
                        {explanation.length >= 50 && <Zap className="w-3 h-3 ml-1 animate-pulse" />}
                      </Button>
                      <Button 
                        onClick={simplifyExplanation}
                        disabled={isSimplifying || explanation.length < 50}
                        variant="outline"
                        size="sm"
                        className={`transition-all duration-300 hover:scale-105 hover:shadow-lg ${
                          explanation.length >= 50 
                      ...
                      >
                        ...
                        Simplify
                        {explanation.length >= 50 && <Rocket className="w-3 h-3 ml-1 animate-pulse" />}
                      </Button>
                    </div>
